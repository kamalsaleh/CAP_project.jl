...
macro InstallMethod(operation::Symbol, filter_list, func)
	...
	@assert (filter_list === :nothing || (filter_list isa Expr && filter_list.head === :vect && all(f -> f isa Symbol, filter_list.args))) "Trying to install $(operation) with the filter_list: $(filter_list)"	
