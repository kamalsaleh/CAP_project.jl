...
InstallMethodWithCache( AdditiveFunctorDataFromValuesOnBasisMorphisms,
  ...
  @FunctionWithNamedArguments(
    ...
    if (@not IsLinearCategoryOverCommutativeRingWithFinitelyGeneratedFreeExternalHoms( source_cat ))
      Error( "the source category must be linear over a commutative ring and have finitely generated free external homs!\n" );
    end;

    if (!( ForAll( [ source_cat, target_cat ], IsLinearCategoryOverCommutativeRing )
              && IsIdenticalObj( CommutativeSemiringOfLinearCategory( source_cat ), CommutativeSemiringOfLinearCategory( target_cat ) ) ))
      Error( "the source and target categories must be linear over the same commutative ring!\n" );
    end;
    
    values_on_objects = known_values_on_objects;
    
    if (values_on_objects == fail)
      values_on_objects = [[],[]];
    end;
    ...
    cached_position =
      function( list, object )
        return PositionProperty( list, x -> IsIdenticalObj( x, object ) || IsEqualForObjects( x, object ) );
    end;
    
    cached_target_position =
      function( list, object )
        return PositionProperty( list, x -> IsIdenticalObj( x[1], object ) || IsEqualForObjects( x[1], object ) );
    end;
    ...
    if (is_full)
      ...
      preimage_morphism_function =
        function( S, phi, T )
          ...
          if (preimage_data[1][index_S] == fail)
          ...
          if (preimage_data[2][index_S][index_T] == fail)
