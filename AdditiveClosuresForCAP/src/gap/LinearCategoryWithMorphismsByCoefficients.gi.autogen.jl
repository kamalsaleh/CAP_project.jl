...
@InstallMethod( LinearCategoryWithMorphismsByCoefficients,
 ...
 @FunctionWithNamedArguments(
  ...
  function ( CAP_NAMED_ARGUMENTS, L, options )
    ...
    @Assert( 0, HasIsLinearCategoryOverCommutativeSemiringWithFinitelyGeneratedFreeExternalHoms( L ) &&
            IsLinearCategoryOverCommutativeSemiringWithFinitelyGeneratedFreeExternalHoms( L ) );
    
    @Assert( 0, HasRangeCategoryOfHomomorphismStructure( L ) &&
    ...
    modeling_tower_morphism_datum =
      function ( Coeff, morL )
        ...
        return PairGAP( Length( multiplicities ), multiplicities );
      ...
      function ( Coeff, source, target )
        ...
        return List( Hom, bas ->
                     MorphismConstructor( Coeff,
                             source,
                             PairGAP( l, EntriesOfHomalgRowVector( MorphismDatum( H, bas ) ) ),
                             target ) );
    ...
    AddZeroMorphism( Coeff,
      function ( Coeff, source, target )
        ...
        return MorphismConstructor( Coeff,
                       source,
                       PairGAP( l, ListWithIdenticalEntries( l, Zero( k ) ) ),
                       target );
    ...
    AddAdditionForMorphisms( Coeff,
      function ( Coeff, mor1, mor2 )
        ...
        return MorphismConstructor( Coeff,
                       Source( mor1 ),
                       PairGAP( l, List( (1):(l), i -> coeffs1[i] + coeffs2[i] ) ),
                       Target( mor1 ) );
    ...
    AddMultiplyWithElementOfCommutativeSemiringForMorphisms( Coeff,
      function ( Coeff, r, mor )
        ...
        return MorphismConstructor( Coeff,
                       Source( mor ),
                       PairGAP( l, List( (1):(l), i -> r * coeffs[i] ) ),
                       Target( mor ) );
        
    end );
    
    if (CanCompute( L, "AdditiveInverseForMorphisms" ))
    ...
    if (CanCompute( L, "SubtractionForMorphisms" ))
    ...
    AddPreCompose( Coeff,
      function ( Coeff, mor_pre, mor_pst )
        ...
        return MorphismConstructor( Coeff,
                       source,
                       PairGAP( l, List( (1):(l), i -> Sum( cmp, c -> c[i] ) ) ),
                       target );
