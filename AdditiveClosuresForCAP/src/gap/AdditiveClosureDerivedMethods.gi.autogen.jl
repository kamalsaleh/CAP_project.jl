...
AddDerivationToCAP( WeakKernelEmbedding,
  ...
  function ( cat, phi )
    ...
    tau = @Concatenation( List( objs, o ->
                @Concatenation(BasisOfSolutionsOfHomogeneousLinearSystemInLinearCategory( cat, [ [  IdentityMorphism( cat, o ) ] ], [ [ phi ] ] ) ) ) );
    ...
    return UniversalMorphismFromDirectSumWithGivenDirectSum( cat, diagram, Source( phi ), tau, S );
        ...
        CategoryFilter = cat -> HasUnderlyingCategory( cat ) && HasIsObjectFiniteCategory( UnderlyingCategory( cat ) ) && IsObjectFiniteCategory( UnderlyingCategory( cat ) )
);
...
AddDerivationToCAP( WeakCokernelProjection,
  ...
  function ( cat, phi )
    ...
    tau = @Concatenation( List( objs, o ->
                @Concatenation( BasisOfSolutionsOfHomogeneousLinearSystemInLinearCategory( cat, [ [ phi ] ], [ [  IdentityMorphism( cat, o ) ] ]  ) ) ) );
